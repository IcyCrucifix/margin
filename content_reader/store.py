...
class VaultStore:
    ...
    def vault_health(self) -> dict[str, Any]:
        """Prove that the configured directory is an Obsidian vault we can read and write."""
        checked_at = _now_iso()
        result: dict[str, Any] = {
            "connected": False,
        ...
        try:
            if not self.vault.is_dir():
                result["reason"] = "The configured vault folder is unavailable."
                return result
            result["obsidian_detected"] = (self.vault / ".obsidian").is_dir()
            if not result["obsidian_detected"]:
            ...
            if not result["read_verified"]:
                result["reason"] = "The vault write check could not be read back."
                return result
            result["connected"] = True
            result["reason"] = "Obsidian detected; read and write verified."
            return result
        except OSError as exc:
            result["reason"] = f"Vault verification failed: {exc.strerror or exc}"
            return result
    ...
    def _raw_note_template(self, record: dict[str, Any]) -> str:
        source_target = self._wiki_target(record["source_path"], keep_suffix=True)
        polished_target = self._wiki_target(record["polished_note_path"])
        ...
        lines = [
            ...
            "",
            "> [!abstract] Lecture file",
            f"> Course: **{record['course']}**  ",
            f"> Source: [[{source_target}|{source_label}]]  ",
            f"> Polished note: [[{polished_target}|Open polished note]]",
            "",
            "> [!tip] Page matching",
        ...
        for page_number in range(1, record["page_count"] + 1):
            ...
            if record["kind"] == "pdf":
                lines.extend(
                    [
                        f"![[{source_target}#page={page_number}]]",
                        "",
                    ]
                )
            elif page_number == 1:
                lines.extend(
                    [
                        f"[[{source_target}|Open the original PowerPoint]]",
                        "",
                    ]
                )
            lines.extend(
    ...
    def _extracted_note_template(
        ...
        lines = [
            ...
            "",
            "> Machine-extracted text for the nightly polishing workflow. The original lecture file remains authoritative.",
            "",
        ]
    ...
    def finalize_polished(
        ...
        raw_target = self._wiki_target(record["raw_note_path"])
        source_target = self._wiki_target(record["source_path"], keep_suffix=True)
        polished_path = self.vault / record["polished_note_path"]
        content = "\n".join(
            [
                ...
                f"# {record['title']}",
                "",
                "> [!abstract] Lecture links",
                f"> Course: **{record['course']}**  ",
                f"> Original source: [[{source_target}|Open lecture file]]  ",
                f"> Class memos: [[{raw_target}|Open page-linked raw notes]]",
                "",
                body,
                "",
            ]
    ...
    def write_hub(self, documents: list[dict[str, Any]]) -> Path:
        ...
        for course in sorted(courses, key=str.casefold):
            ...
            for document in sorted(
                ...
                raw = self._wiki_target(document["raw_note_path"])
                polished = self._wiki_target(document["polished_note_path"])
                source = self._wiki_target(document["source_path"], keep_suffix=True)
                polished_exists = (self.vault / document["polished_note_path"]).exists()
                polished_label = "Open" if polished_exists else "Pending"
                lines.append(
                    f"| {document['lecture_date']} | {document['title']} | [[{raw}|Raw]] | [[{polished}|{polished_label}]] | [[{source}|{document['kind'].upper()}]] |"
                )
            lines.append("")
