...
- **Polish now:** open Stage 2 options for the currently selected lecture.
- **Polish pending:** open Stage 2 options for every missing or outdated lecture.
- **Optional daily scheduler:** run the pending queue at a configured local time while the Margin server is running.

The Stage 2 options include one-click buttons to copy a one-time polishing prompt or a nightly-automation prompt into the user’s own AI system. The prompt text is generated by Margin and is not displayed in the interface.
...
When a user presses a Stage 2 action, Margin checks whether Codex is installed and signed in. If direct polishing is unavailable, it displays:
...
The user can then press **Copy one-time polish prompt** or **Copy nightly automation prompt**. They do not need to open the code or edit `config.json` to use these fallback prompts.

Advanced users can still connect another local AI directly:
