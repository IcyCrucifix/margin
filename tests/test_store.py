...
class VaultStoreTest(unittest.TestCase):
    ...
    def test_pptx_extracts_pages_and_uses_central_fallback(self) -> None:
        ...
        self.assertTrue(page.exists())
        self.assertFalse((self.vault / record["raw_note_path"]).exists())
        ...
        self.assertFalse((self.vault / moved["raw_note_path"]).exists())
        self.store.save_note(moved["id"], 1, "A memo after course routing.")
