...
Margin 是一个本地优先的课堂学习工作区，让你直接在 PDF 讲义或 PowerPoint 幻灯片旁记录笔记。每条 Markdown/LaTeX 备忘都会与它所对应的页面精确关联，让笔记在下课很久之后仍然保留完整语境。

[访问网站](https://icycrucifix.github.io/margin/) · [安装 Margin](#快速上手) · [阅读文档](#文档)


## 为什么选择 Margin？

讲义和独立的笔记文档很容易失去对应关系：课堂上记下的公式当时很清楚，几周后却难以记起它对应哪张图、哪个定义或哪个例题。Margin 让两者始终紧密相连。

- **边看边记。** 阅读 PDF 或 PowerPoint 时，为每页或每张幻灯片撰写自动保存的备忘；只有实际写下内容后，Margin 才会创建原始笔记文件。
- **自然地写作。** 使用 Markdown、即时渲染的数学公式，以及 `\omega` 和 `\Omega` 等 LaTeX 快速建议。
- **掌握自己的文件。** 原始讲义保持不变，所有讲义和笔记都留在你的电脑上。
- **决定笔记的存放位置。** 使用普通文件夹，或将内容直接导入现有的 Obsidian 仓库。
- **课后再完成整理。** 可选的 Stage 2 能够将讲义和课堂上的草稿备忘合并成结构化笔记。

![包含 Markdown、公式和自动保存状态的 Margin 逐页备忘](site/assets/margin-notes.png)

## 从课堂讲义到完整笔记

1. **打开讲义。** 导入 `.pdf` 或 `.pptx`；Margin 会在本地准备页面，不会修改原文件。
2. **记下重点。** 上课时在每一页旁记录想法，每条备忘都会自动保存在正确的语境中。
3. **随时整理。** 保留原始笔记，或在之后使用你选择的 AI 工具生成结构化的完善笔记。

Obsidian 并非必需。普通文件夹模式使用可移植的 Markdown 链接；Obsidian 模式则添加 Wiki 链接、PDF 页面嵌入和课程文件夹路由。
...
安装并启动 Margin 后，便可直接使用 <http://127.0.0.1:4317> 上的本地界面，也可以打开 <https://icycrucifix.github.io/margin/workspace/> 上的网页界面。二者使用同一个本地配套服务和本地资料库。
...
Chrome 只能通过回环地址访问这台电脑上的 Margin。本地配套服务只会原地读取用户明确打开的讲义文件和文件夹；写入笔记文件夹或 Obsidian 仓库的只有原始笔记、润色笔记和 Margin 元数据。GitHub 无法获取讲义文件、笔记、仓库路径、会话令牌或文件系统访问权限。
...
1. 选择 **Open file or folder**（打开文件或文件夹），然后选择一个 `.pdf`、`.pptx` 或包含支持讲义的文件夹。
2. 输入课程代码、讲义标题和日期。
3. 选择缩略图，或聚焦查看器后按 **左/右方向键** 或 **上/下方向键**，即可切换页面。
4. 在右侧编辑器中编写 Markdown。`$...$` 和 `$$...$$` 数学公式会就地渲染。
5. 输入 `\`，再输入符号名称，即可获得 `\omega` 和 `\Omega` 等 LaTeX 建议。

笔记会自动保存。打开讲义时，Margin 不会创建空白原始笔记；第一条非空备忘保存后才会生成 Markdown 原始笔记，并将每条备忘保存在稳定的页面标记之间。讲义源文件保留在原磁盘位置，绝不会复制到 Obsidian。
从 Margin 中移除文件或文件夹只会改变资料库显示，不会删除磁盘源文件、原始笔记或润色笔记；再次打开同一源文件会重新连接已有笔记记录。
如果某个页面或幻灯片无法渲染，请在查看器工具栏中选择 **Reload file**（重新加载文件）。Margin 会重新尝试生成讲义图像，而不会改变原始文件，也不会改变备忘与页面之间的对应关系。
