# Obsidian mode — how Margin writes into a vault
...
- Use `storage_mode: "obsidian"` and set `vault_path` in `config.json` to a folder containing `.obsidian/` (open it in Obsidian once to create it). The health check verifies: folder exists → `.obsidian` present → a write+read-back proof file succeeds.
...
1. Copy `config.obsidian.example.json` to `config.json`.
2. Open (or create) the vault in Obsidian once, so `.obsidian/` exists.
3. Point `vault_path` at it; pick a `notes_root` name that doesn't collide with an existing folder you use for something else.
4. Start the server and confirm the storage card says read/write verified.
5. Optional: create course-code folders (e.g. `COMP1010/`) anywhere in the vault to route those courses' notes there.
