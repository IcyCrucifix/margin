...
async function checkVaultConnection() {
  ...
  elements.vaultStatusText.textContent = "Checking Obsidian…";
  elements.vaultProof.textContent = "Testing vault read and write access";
  try {
    const health = await api("/api/health");
    elements.vaultLabel.textContent = health.vault_name || "Configured vault";
    elements.vaultStatusCard.title = `${health.vault_path}\n${health.reason}\nClick to verify again`;
    elements.vaultStatusCard.classList.remove("checking");
    if (health.connected) {
      elements.vaultStatusCard.classList.add("connected");
      elements.vaultStatusText.textContent = "Obsidian verified";
      elements.vaultProof.textContent = "Vault detected · Read/write passed · just now";
    } else {
      elements.vaultStatusCard.classList.add("disconnected");
      elements.vaultStatusText.textContent = "Obsidian unavailable";
      elements.vaultProof.textContent = health.reason || "Vault verification failed";
    }
    return health;
  } catch (error) {
    elements.vaultStatusCard.classList.remove("checking");
...
async function selectDocument(id, saveFirst = true) {
  ...
  elements.polishButton.disabled = !doc.has_notes;
...
async function importLecture(event) {
  ...
  try {
    ...
    elements.importDialog.close();
    showToast(`${document.title} is ready. Raw notes are linked in Obsidian.`);
    state.pendingFile = null;
    elements.fileInput.value = "";
...
elements.polishButton.addEventListener("click", polishNow);
